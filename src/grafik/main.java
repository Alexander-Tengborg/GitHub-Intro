	...
	private void makeView4() {
		...
		for (int row = 0; row < rows; row++) {
			...
			for (int col = 0; col < cols; col++) {
				...
				// Skapa en ny Circle
				Circle rect = new Circle(radius);

				// Flytta cir i x-led så den kommer rätt
				rect.setTranslateX(col * width + radius);

				// Flytta cir i y-led så den kommer rätt
				rect.setTranslateY(row * height + radius);
